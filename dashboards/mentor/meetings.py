...
from database.meetings import delete_meeting, list_meetings, save_meeting, save_recurring_meetings, update_meeting
...
def render_mentor_meetings():
    ...
    with st.expander("＋ Schedule or change a meeting", expanded=False):
        ...
        with hour:
            value_hour = st.selectbox("Hour", list(range(1, 13)), index=8, key="meeting_hour")
        with minute:
            value_minute = st.selectbox("Minute", [0, 15, 30, 45], key="meeting_minute")
        with meridiem:
            value_meridiem = st.selectbox("AM / PM", ["AM", "PM"], index=1, key="meeting_meridiem")
        ...
        if st.button("Schedule meeting", use_container_width=True):
            if not student or not title.strip() or not link.strip():
                ...
                st.success("12 Tuesday/Thursday meetings scheduled.")
                st.rerun()
            else:
    ...
    for meeting in meetings:
        with st.container(border=True):
            ...
            with open_col:
                if link and st.button("🖥️ Show meeting here", key="mentor_show_{}".format(meeting["id"])):
                    st.session_state["active_mentor_meeting_link"] = link
                    st.rerun()
            with delete_col:
                if st.button("🗑️ Delete meeting", key="delete_meeting_{}".format(meeting["id"]), type="secondary"):
            ...
            with st.expander("Edit meeting"):
                ...
                if st.button("Save meeting changes", key="save_meeting_{}".format(meeting["id"])):
                    update_meeting(meeting["id"], changed_title, changed_date, changed_time, changed_link)
                    st.rerun()
